...
const PeriodoCalificaciones = ({ periodo, idMateriaPlanCiclo, userRole }) => {
  ...
  const { data: calificaciones, isLoading } = useQuery({
    queryKey: [
      "materia-plan-ciclo-calificaciones",
      idMateriaPlanCiclo,
      periodo,
    ],
    queryFn: async () => {
      const response = await api.get(
        `/admin/materia/materia-plan-ciclo/${idMateriaPlanCiclo}/calificaciones/${periodo}`
      );
  ...
  const actualizarCalificacion = useMutation({
    mutationFn: async ({ inscripcionId, calificacion }) => {
      await api.put(
        `/admin/materia/materia-plan-ciclo/calificaciones/${inscripcionId}`,
        {
          calificacion,
                  ...
                  {isEditing ? (
                    ...
                    canEdit && (
                      <Boton
                        variant="primary"
                        onClick={() =>
                          handleEdit(cal.inscripcionId, cal.calificacion)
                        }
                        title={
                          cal.bloqueada
