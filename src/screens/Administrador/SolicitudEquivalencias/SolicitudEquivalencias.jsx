...
const SolicitudEquivalencias = () => {
  ...
  const handleAceptarSolicitud = async (datos) => {
    try {
      await api.put(`/equivalencia/admin/${solicitudSeleccionada.id}/aprobar`, datos)
      // Recargar las solicitudes
      await cargarSolicitudes()
      setModalAceptarOpen(false)
    } catch (err) {
      throw new Error(err.response?.data?.message || 'Error al aprobar la solicitud')
    }
  }

  const handleRechazarSolicitud = async (motivoRechazo) => {
    try {
      await api.put(`/equivalencia/admin/${solicitudSeleccionada.id}/rechazar`, { motivoRechazo })
      // Recargar las solicitudes
      await cargarSolicitudes()
      setModalRechazarOpen(false)
    } catch (err) {
      throw new Error(err.response?.data?.message || 'Error al rechazar la solicitud')
    }
  }
      ...
      <div className={styles.content}>
        <div className={styles.controls}>
          ...
          <div className={styles.filtros}>
            <button
              className={filtroEstado === 'Todos' ? styles.filtroActivo : styles.filtro}
              onClick={() => handleFiltroEstado('Todos')}
            >
              Todos ({solicitudes.length})
            </button>
            <button
              className={filtroEstado === 'Pendiente' ? styles.filtroActivo : styles.filtro}
              onClick={() => handleFiltroEstado('Pendiente')}
            >
              Pendientes ({solicitudes.filter(s => s.estado === 'Pendiente').length})
            </button>
            <button
              className={filtroEstado === 'Aprobada' ? styles.filtroActivo : styles.filtro}
              onClick={() => handleFiltroEstado('Aprobada')}
            >
              Aprobadas ({solicitudes.filter(s => s.estado === 'Aprobada').length})
            </button>
            <button
              className={filtroEstado === 'Rechazada' ? styles.filtroActivo : styles.filtro}
                ...
                <div className={styles.cardHeader}>
                  <div className={styles.alumnoInfo}>
                    <h3 className={styles.alumnoNombre}>
                      {solicitud.alumno?.persona?.apellido}, {solicitud.alumno?.persona?.nombre}
                    </h3>
                    <span className={styles.alumnoDni}>DNI: {solicitud.alumno?.persona?.dni || '-'}</span>
                  </div>
                  <span className={`${styles.estadoBadge} ${getEstadoBadgeClass(solicitud.estado)}`}>
                    {solicitud.estado}
                  </span>
                </div>
                ...
                <div className={styles.cardBody}>
                  <div className={styles.infoRow}>
                    <div className={styles.infoItem}>
                      <span className={styles.infoLabel}>Institución de Origen</span>
                      <span className={styles.infoValue}>{solicitud.origen_institucion}</span>
                    </div>
                    <div className={styles.infoItem}>
                      <span className={styles.infoLabel}>Materia de Origen</span>
                      <span className={styles.infoValue}>{solicitud.origen_materia}</span>
                    </div>
                  </div>

                  <div className={styles.infoRow}>
                    <div className={styles.infoItem}>
                      <span className={styles.infoLabel}>Calificación Obtenida</span>
                      <span className={styles.infoValueHighlight}>{solicitud.origen_calificacion}</span>
                    </div>
                    <div className={styles.infoItem}>
                      <span className={styles.infoLabel}>Resolución Plan</span>
                      <span className={styles.infoValue}>{solicitud.resolucion || '-'}</span>
                    </div>
                  </div>
                  ...
                  <div className={styles.infoRow}>
                    <div className={styles.infoItem}>
                      ...
                      <span className={styles.infoValue}>{formatFecha(solicitud.fecha_solicitud)}</span>
                    </div>
                    {solicitud.fecha_resolucion && (
                      <div className={styles.infoItem}>
                        <span className={styles.infoLabel}>Fecha de Resolución</span>
                        <span className={styles.infoValue}>{formatFecha(solicitud.fecha_resolucion)}</span>
                      </div>
                    )}
                  </div>
                  ...
                  {solicitud.motivo_rechazo && (
                    <div className={styles.motivoRechazo}>
                      ...
                      <p className={styles.motivoTexto}>{solicitud.motivo_rechazo}</p>
                    </div>
                  )}
                </div>
